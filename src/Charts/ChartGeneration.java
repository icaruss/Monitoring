...
public class ChartGeneration extends ApplicationFrame {
	 ...
	 */
	public ChartGeneration(String name, int _Seconds[], int[] _Minutes,
			int[] _Hours, int[] _Days, int[] _Months, int[] _Years,
			Double[] _CPU, String chartName) throws IOException {
		super(name);
		Seconds = _Seconds;
		Minutes = _Minutes;
		Hours = _Hours;
	...
	public ChartGeneration(String name, int _Seconds[], int[] _Minutes,
			int[] _Hours, int[] _Days, int[] _Months, int[] _Years,
			Double[] _VSZ, Double[] _RSS, String chartName) throws IOException {
		super(name);
		Seconds = _Seconds;
		Minutes = _Minutes;
		Hours = _Hours;
	...
	private JFreeChart createChart(XYDataset xydataset, String CPU_MEM) {
		JFreeChart jfreechart = ChartFactory.createTimeSeriesChart(
				"Monitoring Results", "DateTime", CPU_MEM, xydataset, true,
				true, false);
		...
		periodaxis
				.setAutoRangeTimePeriodClass(org.jfree.data.time.Second.class);
		periodaxis
				.setMajorTickTimePeriodClass(org.jfree.data.time.Second.class);
		PeriodAxisLabelInfo aperiodaxislabelinfo[] = new PeriodAxisLabelInfo[2];
		aperiodaxislabelinfo[0] = new PeriodAxisLabelInfo(
				org.jfree.data.time.Second.class, new SimpleDateFormat(
						"HH:mm:ss"), new RectangleInsets(2D, 2D, 2D, 2D),
				new Font("SansSerif", 1, 14), Color.blue, false,
				new BasicStroke(0.0F), Color.lightGray);
		aperiodaxislabelinfo[1] = new PeriodAxisLabelInfo(
				org.jfree.data.time.Day.class, new SimpleDateFormat(
						"dd-MM-yyyy"), new RectangleInsets(2D, 2D, 2D, 2D),
				new Font("SansSerif", 1, 14), Color.blue, false,
				new BasicStroke(0.0F), Color.lightGray);
	...
	private XYDataset createDatasetMem() {
		...
		if (RSSMemArr == null) {
			...
			for (int i = 0; i < Seconds.length; i++) {
				...
				timeseries.add(new Second(Seconds[i], Minutes[i], Hours[i],
						Days[i], Months[i], Years[i]), VSZMemArr[i]);
			}
			...
			for (int i = 0; i < Seconds.length || i < RSSMemArr.length; i++) {
				...
				// TODO: To separate between HP-UX & other OS..
				timeseries.add(new Second(Seconds[i], Minutes[i], Hours[i],
						Days[i], Months[i], Years[i]), RSSMemArr[i]);
				timeseries1.add(new Second(Seconds[i], Minutes[i], Hours[i],
						Days[i], Months[i], Years[i]), VSZMemArr[i]);
			}
	...
	private XYDataset createDatasetCPU() {
		...
		for (int i = 0; i < Seconds.length; i++) {
			...
			// TODO: To separate between HP-UX & other OS..
			timeseries.add(new Second(Seconds[i], Minutes[i], Hours[i],
					Days[i], Months[i], Years[i]), CPU[i]);
			// timeseries1.add(new
			// Second(Seconds[i],Minutes[i],Hours[i],Days[i],Months[i],
			// Years[i]),129 +i+2);
		}
	...
	/*
	 * public JPanel createDemoPanel() { if (CPU == null) { JFreeChart
	 * jfreechart = createChart(createDatasetCPU(),"CPU"); return new
	 * ChartPanel(jfreechart); } else { JFreeChart jfreechart =
	 * createChart(createDatasetMem(), "Memory"); return new
	 * ChartPanel(jfreechart); } }
	 */

	/*
	 * public static void main(String args[]) { ChartGeneration periodaxisdemo1
	 * = new ChartGeneration("Period Axis Demo"); periodaxisdemo1.pack();
