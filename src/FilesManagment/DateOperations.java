...
public class DateOperations extends FilesManagment {
	 ...
	 * @return the date[]
	 */
	public Date[] promoteTimeWithInterval(Date date, int interval,
			int fileLineNum) {
	 ...
	 *             the parse exception
	 */
	public String[] convertDateArrToStrinArr(Date[] dateArr)
			throws ParseException {
	 ...
	 * @return the list
	 */
	public List<Integer> findAllDatesInInterval(Date date1, int interval,
			Date[] clixDateArr) {
		Date date2 = new Date();
		...
		for (int i = 0; i < clixDateArr.length; i++) {
			...
			if (clixDateArr[i].equals(date1)
					|| clixDateArr[i].equals(date2)
					|| (clixDateArr[i].after(date1) && clixDateArr[i]
							.before(date2))) {
				list.add(i + 2);
			}
		}
	...
	public Date getDateFromFile(String FileName, Boolean hasInstance) {
		...
		try {
			...
			if ((new String(dest).equalsIgnoreCase("mdis"))
					|| (new String(dest).equalsIgnoreCase("mdss"))) {

				dest = new char[10];
				ExtractedfileName
						.getChars(9 + instance, 19 + instance, dest, 0); // get
																			// date
				String strFullDate = new String(dest);
				dest = new char[8];
				ExtractedfileName.getChars(20 + instance, 28 + instance, dest,
						0); // get time
				...
				dest = new char[10];
				ExtractedfileName
						.getChars(8 + instance, 18 + instance, dest, 0); // get
																			// date
				String strFullDate = new String(dest);
				dest = new char[8];
				ExtractedfileName.getChars(19 + instance, 27 + instance, dest,
						0); // get time
				strFullDate = strFullDate + " " + new String(dest);
				DateFormat formatter;
				formatter = new SimpleDateFormat("dd-MM-yyyy HH-mm-ss");
				date = formatter.parse(strFullDate);
				// System.out.println(formatter.format(date)); //Prints the date
				// in the format of dd-mm-yyyy HH-mm-ss
			}
	 ...
	 * @return the int[]
	 */
	public int[] generateSecondArray(Date[] dateArr) // dateArr is generated
														// from
														// promoteTimeWithInterval()
														// method
	{
